<template>
  <el-dialog v-model="dialogFormVisible" title="" width="700">
    <div style="border: 1px solid #ccc">
      ...
      />
      <Editor
        style="height: 490px; overflow-y: hidden"
        v-model="valueHtml"
...
bus.on("sendValueId", async (value) => {
  ...
  if (value == 8) {
    valueHtml.value =
      (await getAllCompanyIntroduce()).data.results[1].set_text || "";
  } else if (value == 9) {
    valueHtml.value =
      (await getAllCompanyIntroduce()).data.results[2].set_text || "";
  } else if (value == 10) {
    valueHtml.value =
      (await getAllCompanyIntroduce()).data.results[3].set_text || "";
  } else if (value == 11) {
    valueHtml.value =
      (await getAllCompanyIntroduce()).data.results[4].set_text || "";
  }
